...
export default function App() {
  ...
  const [currentChatId, setCurrentChatId] = useState(() => {
    const saved = localStorage.getItem('ins_currentChatId');
  ...
  const [editMsg, setEditMsg] = useState(null); // 正在编辑的用户消息
  ...
  useEffect(() => { localStorage.setItem('ins_theme', theme); applyTheme(theme); }, [theme]);
  useEffect(() => { localStorage.setItem('ins_uiScale', uiScale); applyScale(); }, [uiScale, fontSize]);
  useEffect(() => { localStorage.setItem('ins_fontSize', fontSize); applyScale(); }, [fontSize, uiScale]);
  useEffect(() => { if (customBg) localStorage.setItem('ins_customBg', customBg); else localStorage.removeItem('ins_customBg'); }, [customBg]);
  useEffect(() => { localStorage.setItem('ins_appWidth', appWidth); localStorage.setItem('ins_appHeight', appHeight); }, [appWidth, appHeight]);
  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [conversations[currentChatId]?.messages]);
  ...
  const sendMessage = async (text = null) => {
    ...
    if (chat.messages.length === 0) chat.title = inputText.slice(0, 25) + (inputText.length > 25 ? '...' : '');
    setConversations(prev => ({ ...prev, [currentChatId]: { ...prev[currentChatId], messages: newMessages, history: newHistory, title: prev[currentChatId].title } }));
    document.getElementById('userInput').value = '';
    setIsTyping(true);

    // 模拟 AI 回复（将来替换为 fetch）
    try {
      ...
      setConversations(prev => {
        const updatedChat = { ...prev[currentChatId] };
        updatedChat.messages = [...updatedChat.messages, aiMsg];
        updatedChat.history = [...updatedChat.history, { role: 'assistant', content: replyText }];
  ...
  const regenerate = async () => {
    ...
    setConversations(prev => ({ ...prev, [currentChatId]: { ...prev[currentChatId], messages: newMsgs, history: newHistory } }));
  ...
  const submitEdit = async () => {
    ...
    setConversations(prev => ({ ...prev, [currentChatId]: { ...prev[currentChatId], messages: newMessages, history: newHistory } }));
    setEditMsg(null);
  ...
  // 删除对话
  const deleteChat = (id) => {
    if (Object.keys(conversations).length <= 1) return showToast('至少保留一个对话');
    setConversations(prev => {
      const copy = { ...prev };
      delete copy[id];
      return copy;
    ...
    if (currentChatId === id) {
      const nextId = Object.keys(conversations)[0];
      if (nextId) setCurrentChatId(nextId);
    }
    showToast('对话已删除');
  };
  ...
  const handleFileImport = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (messages.length === 0) return showToast('未识别到任何聊天记录');
        const id = Date.now().toString(36) + Math.random().toString(36).substr(2, 6);
        const newChat = {
  ...
  const saveSettings = () => {
    ...
    const autoSpeak = document.getElementById('setAutoSpeak').checked;
    ...
    setAgentConfig({ ...agentConfig, name, prompt, autoSpeak, maxTokens, temperature, voiceIndex: voiceIdx, avatar: agentConfig.avatar });
    setUiScale(scale);
    setFontSize(size);
  ...
  const clearConversation = () => {
    if (isTyping) return showToast('请先停止生成');
    setConversations(prev => ({ ...prev, [currentChatId]: { ...prev[currentChatId], messages: [], history: [], title: '新对话' } }));
    setSettingsOpen(false);
    showToast('🗑️ 对话已清空');
    setTimeout(() => {
      const aiMsg = { role: 'ai', content: '嗨！我是你的智能助手，有什么想聊的吗？🌿', time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
      setConversations(prev => ({ ...prev, [currentChatId]: { ...prev[currentChatId], messages: [aiMsg], history: [{ role: 'assistant', content: aiMsg.content }] } }));
    }, 300);
  };
  ...
  return (
    ...
    <input type="file" ref={avatarInputRef} accept="image/*" style={{ display: 'none' }} onChange={(e) => {
      ...
      reader.onload = (ev) => {
        ...
        img.onload = () => {
          ...
          setAgentConfig({ ...agentConfig, avatar: canvas.toDataURL('image/jpeg', 0.8) });
        };
    ...
    <style>{`
      :root {
        ...
        --surface-glass: rgba(255, 252, 248, 0.5);
        ...
        --pearl-shine: rgba(255, 250, 244, 0.9);
        ...
        --font-size-base: 15px;
        --ui-scale: 1;
        --handle-color: rgba(160, 140, 120, 0.3);
        --handle-hover: rgba(140, 110, 90, 0.5);
      ...
      [data-theme="dark"] {
        ...
        --surface-glass: rgba(32, 30, 28, 0.6);
        ...
        --shadow-popup: 0 8px 32px rgba(0, 0, 0, 0.35);
        --accent: #c49b9b;
        --accent2: #9b95d4;
        --accent-warm: #b8957e;
        --pearl-base: rgba(38, 35, 32, 0.7);
        --pearl-shine: rgba(50, 46, 40, 0.8);
        ...
        --toast-color: #1c1a18;
        --handle-color: rgba(120, 105, 90, 0.4);
        --handle-hover: rgba(160, 140, 120, 0.6);
      ...
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      @keyframes mistFloat {
        ...
        50% { opacity:0.9; transform:translateX(18px); }
      }
